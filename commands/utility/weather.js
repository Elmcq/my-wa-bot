...
export default {
    ...
    execute: async (sock, msg, from, args, db) => {
        try {
            ...
            const url =
                "https://api.openweathermap.org/data/2.5/weather?q=Surabaya&units=metric&lang=id&appid=bd4b4d1db50784be9efbeea4df537945";
            ...
            if (temp > 32)
                saran =
                    "⚠️Panas kenthang-kenthang! Ojok lali ngombe es teh ben gak semaput.☀️";
            if (desc.includes("hujan"))
                saran =
                    "⛈️Udan deres rek, ati-ati lunyu lan banjir. Sedia mantel!";
            if (weather.main === "Clouds")
                saran = "☁️Mendung syahdu, enak gae turu.";
            if (temp < 25) saran = "🥶Adem ayem, wayahe kemulan.";

            // Formatting Output with Template Literals (Backticks)
            const caption =
                `☁️ *PRAKIRAAN CUACA SURABAYA* ☁️\n\n` +
                `🌡️ *Suhu:* ${temp}°C\n` +
            ...
            return sock.sendMessage(from, {
                text: "❌ Gagal mengambil data cuaca Surabaya. Coba lagi nanti!",
            });
        }
    },
};
