...
async function startBot() {
    ...
    // Start Ramadan Schedulers (Sahur & Iftar)
    startRamadanSchedulers(sock);
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        // Auto-Reply Logic
        const msgClean = body.toLowerCase().trim();
        if (autoReplies[msgClean]) {
            await sock.sendMessage(from, { text: autoReplies[msgClean] }, { quoted: msg });
        }
